...
pub fn main() {
    ...
    let param=HmcParam::quick_adj(0.99);
    for i in 0..10000000 {
        sample(&rosenbrock_f, &diff_rosenbrock, &mut x, &mut lp, &mut rng, &mut epsilon, 2, &param);
    }
    let param=HmcParam::slow_adj(0.99);
    for i in 0..100000000 {
        if sample(&rosenbrock_f, &diff_rosenbrock, &mut x, &mut lp, &mut rng, &mut epsilon, 2, &param){
            accept_cnt+=1;
        }
        if i%100000==0{
