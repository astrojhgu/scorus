...
use rand::thread_rng;
use rand::Rng;
use scorus::mcmc::hmc::naive::{HmcParam, sample};
...
fn logprob(ym: &[f64], x: &[f64], y: &[f64], sigma: &[f64])->f64{
    let p1=x.iter().zip(y.iter().zip(sigma.iter().zip(ym.iter()))).map(|(&x, (&y, (&sigma, &ym)))|{
        -(ym-y).powi(2)/(2.0*sigma.powi(2))
    }).sum::<f64>();
    let n=x.len();

    let y2=ym.windows(3).map(|ym|{
        let y1=ym[0];
        let y2=ym[1];
        let y3=ym[2];
...
fn grad_logprob(ym: &[f64], x: &[f64], y: &[f64], sigma: &[f64])->Vec<f64>{
    ...
    for q in 0..x.len(){
        ...
        for i in 1..x.len()-1{
            result[q]+=-(ym[i+1]+ym[i-1]-2.0*ym[i])*(delta(i+1, q)+delta(i-1, q)-2.0*delta(i, q))/smooth_param;
        }
    }
    result
}
...
fn main(){
    ...
    let y:Vec<_>=x.iter().map(|&x|->f64{ let x1: f64=rng.sample(StandardNormal);
    x1*0.1+(x/10.0).sin()
    }).collect();
    ...
    let mut cnt=0;
    for i in 0..1000000{
        let accepted=sample(&lp_f, &glp_f, &mut q, &mut lp, &mut g, &mut rng, &mut epsilon, 15, &param);
        ...
        if i%1000==0 || i<1000{
            eprintln!("{} {} {} {}", i, accept_cnt as f64/(i+1) as f64, epsilon, lp);
        }
