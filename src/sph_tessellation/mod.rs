    ...
    T: Float + Copy,
{
    pub fn octahedron()->Tessellation<T>{
        ...
        Tessellation{vertices:vertices, faces:faces}
    }
        ...
        for f in &self.faces{
            ...
            for i in 0..3{
                match edge_mid_map.get(&(edges[i])){
                    ...
                    _=>{
                        let mid=normalized_mid_point(&self.vertices[edges[i].0], &self.vertices[edges[i].1]);
            ...
            new_faces.push(regulate([new_vertex_ids[0], new_vertex_ids[1], new_vertex_ids[2]]));
