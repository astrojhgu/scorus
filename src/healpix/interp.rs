...
use super::super::sphcoord::{SphCoord};
...
where T:Float
{
    ...
    let az=z.abs();
    if az<twothird{
        <usize as NumCast>::from(T::from(nside).unwrap()*(T::from(2).unwrap()-T::from(1.5).unwrap()*z)).unwrap()
    }
    else{
        let iring=<usize as NumCast>::from(T::from(nside).unwrap()*(T::from(3.0).unwrap()*(T::one()-az)).sqrt()).unwrap();
        if z>T::zero(){iring}else{4*nside-iring-1}
    }
}
...
where T:Float+FloatConst
{
    ...
    let northring=if ring>2*nside {4*nside-ring} else {ring};
    let mut theta:T;
    let mut ringpix:usize;
    ...
    if northring < nside{
        ...
        theta=((T::from(2).unwrap()*T::from(nside).unwrap()-T::from(northring).unwrap())*fact1).acos();
        ringpix=4*nside;
...
where T:Float+FloatConst
{
    ...
    if ir1>0 {
        ...
        let mut i1 = if tmp < T::zero() { <isize as NumCast>::from(tmp).unwrap() - 1 } else { <isize as NumCast>::from(tmp).unwrap() };
        let w1 = (ptg.az - (T::from(i1).unwrap() + T::from(if shift { 0.5 } else { 0.0 }).unwrap()) * dphi) / dphi;
        let mut i2 = i1 + 1;
        if i1 < 0 { i1 = i1 + nr as isize; }
        if i2 as usize >= nr { i2 = i2 - nr as isize; }
    ...
    if ir2<(4*nside) {
        ...
        if i1 < 0 { i1 += nr as isize; }
        if i2 as usize >= nr { i2 -= nr as isize; }
