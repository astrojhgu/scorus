...
use crate::{
    ...
    healpix::{interp::natural_interp_ring, pix::pix2vec_ring, utils::npix2nside},
};
    ...
    W: Float + FloatConst + Debug,
{
    ...
    let dir: Vec<_> = pix_center_rot
        .iter()
        .map(|&x| SphCoord::from_vec3d(x))
        .collect();

    dir.iter()
        .map(|&d| natural_interp_ring(nside, input, d))
        .collect()
    ...
    W: Float + FloatConst + Debug,
{
    ...
    let nside = npix2nside(npix);
    let rotated_center_dir: Vec<_> = (0..npix)
        .map(|i| mat_inv * pix2vec_ring(nside, i))
        .collect();

    eprintln!("{:?}", rotated_center_dir[0]);

    let rotated_angle: Vec<_> = rotated_center_dir
        .iter()
        .map(|d| SphCoord::from_vec3d(*d))
        .collect();
    let t_rotated: Vec<_> = rotated_angle
        .iter()
        .map(|a| natural_interp_ring(nside, t, *a))
        .collect();
    let mut q_rotated: Vec<_> = rotated_angle
        .iter()
        .map(|a| natural_interp_ring(nside, q, *a))
        .collect();
    let mut u_rotated: Vec<_> = rotated_angle
        .iter()
        .map(|a| natural_interp_ring(nside, u, *a))
        .collect();

    let l_map: Vec<_> = rotated_center_dir
        .iter()
        .zip(q_rotated.iter().zip(u_rotated.iter()))
        .map(|(dir1, (&q1, &u1))| {
            Complex::<T>::new(q1, u1)
                * Complex::<T>::from_polar(T::one(), two * angle_ref(dir1, mat))
        })
        .collect();
    q_rotated
        .iter_mut()
        .zip(u_rotated.iter_mut().zip(l_map.iter()))
        .for_each(|(q, (u, &l))| {
            *q = l.re;
            *u = l.im;
        });
