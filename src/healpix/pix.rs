...
use super::super::sphcoord::{SphCoord, Vec3d};
...
where T:Float+FloatConst{
    ...
    if pix<ncap {
        ...
        let phi=(T::from(iphi).unwrap()-T::one()/T::from(2).unwrap())*half_pi/T::from(iring).unwrap();
        (z, phi)
    }
        ...
        let fodd=if (iring+nside)&1!=0{T::one()}else{T::from(0.5).unwrap()};
        let nl2=2*nside;
        ...
        let z=-T::one()+T::from(iring*iring).unwrap()*fact2;
        let phi=(T::from(iphi).unwrap()-T::from(0.5).unwrap())*half_pi/T::from(iring).unwrap();
        (z, phi)
    }
}
