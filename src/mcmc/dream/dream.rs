...
W: Clone + IndexMut<usize, Output = V> + HasLen + Sync + Send,
{
    ...
    for d in 0..proposed.dimension(){
        if flag[d]{
            proposed[d]=proposed[d]+ dx[d] * gamma*(T::one()+rng.gen_range(-b, b))+rng.sample(StandardNormal)*b_star;
        }
    }
    proposed
}
...
F: Fn(&V) -> T + Send + Sync + ?Sized,
{
    ...
    for (i, (proposed, lp)) in proposed_points.into_iter().zip(next_lp.into_iter()).enumerate(){
        accept(&mut ensemble_logprob.0, &mut ensemble_logprob.1, i, proposed, lp, rng);
    }
}    
