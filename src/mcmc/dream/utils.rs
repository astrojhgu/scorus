...
U: Rng
{
    ...
    let mut p_idx_pair:Vec<_>=logp.iter().enumerate().map(|(i, &lp)| ( (lp-max_lp).exp(), i)).collect();
    ...
    let cum:Vec<_>=p_idx_pair.iter().scan(T::zero(), |st, &x|{
        *st=*st+x.0;
        Some((*st, x.1))
    }).collect();
...
R: Rng,
{
    let result:Vec<_>=(0..ndims).map(|_|{
        rng.gen_range(T::zero(), T::one()) > (T::one()-cr)
    }).collect();
