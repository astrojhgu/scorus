...
use rayon::iter::IndexedParallelIterator;use rayon::iter::IntoParallelRefMutIterator;
    ...
    G: Fn(&V) -> V + Sync,
{
    ...
    sample_ensemble_pt_impl(flogprob, grad_logprob, q0, lp, &mut last_grad_logprob, rng, epsilon, beta_list, l, param)
}
    ...
    G: Fn(&V) -> V + Sync,
{
    ...
    let mut p:Vec<_>=q0.iter().map(|q1| {let mut p=V::zeros_like(q1);
        for i in 0..p.dimension() {
            p[i] = rng.sample(StandardNormal);
        }
        p
    ...
    let mut last_hq_q_tmp:Vec<_> = last_grad_logprob.iter().enumerate().map(|(i, x)| x* (-T::one()*beta_list[i/n_per_beta])).collect() ;

    //q.par_iter_mut().zip(p.par_iter_mut().zip(last_hq_q_tmp.par_iter_mut())).enumerate().for_each(|(i, (q, (p, lhqq)))|{
    q.iter_mut().zip(p.iter_mut().zip(last_hq_q_tmp.iter_mut())).enumerate().for_each(|(i, (q, (p, lhqq)))|{
    ...
    let dh:Vec<_>=current_u.iter().zip(proposed_u.iter().zip(current_k.iter().zip(proposed_k.iter()))).enumerate().map(|(i, (&u0, (&u1, (&k0, &k1))))|{
        let ibeta=i/n_per_beta;
        let beta=beta_list[ibeta];
        beta*(u0-u1)+k0-k1
    }).collect();
    ...
    dh.iter().zip(q0.iter_mut().zip(q.into_iter().zip(lp.iter_mut().zip(last_grad_logprob.iter_mut().zip(proposed_u.into_iter().zip(last_hq_q_tmp.iter())))))).enumerate().for_each(|(i, (&dh1, (q01, (q1, (lp1, (lgl1, (pu, lhqt)))))))|{
        let ibeta=i/n_per_beta;
        //println!("{} {} {}", ibeta, n_per_beta, i);
        if dh1.is_finite() && rng.gen_range(T::zero(), T::one()) < dh1.exp(){
            *q01=q1;
            *lp1 = -pu;
            *lgl1 = lhqt*(-T::one());
